							...
							if(mysqli_num_rows($query) == 0){ //ini artinya jika data hasil query di atas kosong
							   //jika data kosong, maka akan menampilkan row kosong
								...
								while ($data = mysqli_fetch_assoc($query)) {
									...
									echo "<td><a href='#' onclick=\"javascript: if(confirm('Apakah data setoran tabungan dengan ID Tabungan $data[id_tabungan] mau dihapus?')==true){ window.location.href='hapus_setoran.php?aksi=hapus&id_penarikan=$data[id_penarikan]'; } \">Hapus</a></td>";
									echo "</tr>";
									$no++;
								}
							}
